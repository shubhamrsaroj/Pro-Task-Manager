...
// Services 
import { processCompletedRecurringTasks, generateUpcomingRecurringTasks } from './services/recurringTaskService.js';
...
app.get('/api/debug/tasks', authenticateToken, async (req, res) => {
    try {
...
app.get('/api/debug/roles', authenticateToken, async (req, res) => {
    try {
        const { User } = await import('./models/User.js');
        const { getAllPermissions } = await import('./middleware/rbac.js');
        ...
        res.json({
            ...
            roleCount: users.reduce((acc, user) => {
